...
import { ModuleEvents, ClassInfo, FromFiles, Autoinject, Log, ModuleBase, Configuration, DI, Logger, HttpServer, ForbiddenException, Exception, ServerErrorException } from "./index";
...
type RouteCallback = (req: express.Request, res: express.Response, next: express.NextFunction) => (req: express.Request, res: express.Response) => void;
...
export class Controllers extends ModuleBase<ControllerEvents> {
    ...
    public async onInitialize() {
        ...
        for (const controller of controllers) {
            ...
            const controllerName = instance.constructor.name.substring(0, instance.constructor.name.indexOf("Controller")).toLowerCase();

            for (let route of cdesc.Routes) {

                const handlers: RequestHandler[] = [];
                const action: RouteCallback = instance[route.FunctionName];
                const middlewares = await Promise.all<Middleware>(cdesc.Middlewares.concat(route.Middlewares || []).map(m => DI.resolve(m.Middleware, m.Options)));
                ...
                handlers.push(...middlewares.filter(m => m.IsEnabled(route, instance)).map(m => _invokeAction(m.OnBeforeAction.bind(m))));
                ...
                handlers.push(...middlewares.filter(m => m.IsEnabled(route, instance)).map(m => _invokeAction(m.OnAfterAction.bind(m))));
